...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, n_channels=1, zero_init_residual=False,
                 ...
                 norm_layer=None,classification=False, drop_out=0.5):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], drop=drop_out)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], drop=drop_out)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], drop=drop_out)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], drop=drop_out)
        self.drop = nn.Dropout(drop_out) if drop_out > 0.0 else None
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        raise ValueError("pretrained can not be True!")
    return model
...
def resnet152_dropout(pretrained=False, progress=True, **kwargs):
