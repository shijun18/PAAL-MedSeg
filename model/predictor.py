...
class Acc_Predictor(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, input_channels=1, zero_init_residual=False,
                 ...
                 norm_layer=None):
        super(Acc_Predictor, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
def _acc_predictor(block, layers, **kwargs):
    model = Acc_Predictor(block, layers, **kwargs)
    return model


def ap18(**kwargs):
    return _acc_predictor(BasicBlock, [2, 2, 2, 2], **kwargs)